...
// tell tailwind to include style
...
const AngledContentStripe = ({ children, color = 'yellow' }: IProps) => {
    ...
    return (
        <div
            className={`relative m-auto bg-${backgroundClass} mb-[10rem] flex max-w-[1024px]`}
        >
            <svg
            ...
            ></span>
            <span
                className={`absolute bottom-0 right-0 block h-[calc(100%+68px)] w-[calc((100vw-1024px)/2)] translate-x-full bg-${backgroundClass}`}
            ></span>
        </div>
    )
}
